...
-   **Google Sheets Transcript Sync (Phase A):** Syncs leads and transcripts from Google Sheet. DB table `sheet_sync_sessions` tracking sync lifecycle (RUNNING→COMPLETE/PARTIAL/FAILED). New `transcript_text` column on `leadsTable` (nullable text). API routes at `/api/sheet-sync`: `POST /sync` (reads sheet via Google Drive connector, creates/updates leads with fuzzy matching, skips if transcript already populated), `GET /sessions` (list 20 most recent), `GET /sessions/:id`. Fuzzy match extracted to shared lib (`lib/fuzzyMatch.ts`), imported by both `leads/index.ts` and `sheet-sync/index.ts`. Frontend: new "Sync from Google Sheet" tab on `/import` page (parallel state to existing import tab). Changelog actions: `LEAD_CREATED_FROM_SHEET`, `TRANSCRIPT_SYNCED_FROM_SHEET`. triggered_by: `'sheet_sync'`. Uses `connectors.proxy("google-drive", "/v4/spreadsheets/${id}/values/Sheet1")` — Google Drive connector proxies Sheets API v4 reads.
