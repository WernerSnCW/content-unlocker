...
-   **Phase 012a (Work Queue Fixes):** Document-first scope (POST /start builds documentTaskMap from REQUIRES_REVIEW docs directly), task PATCH decoupled from review_state, 6-step verified auto-fix (text exists → different → write → re-read verify → CLEAN → AUTO_FIXED). Doc 110 retroactive fix for 4 AUTO_FIXED findings → FAILED.
-   **Phase 012c (Document Health Check):** New tables (document_health_sessions + document_health_scores), POST /document-health/run + GET /document-health/latest API, 7-dimension analysis (identity, targeting, belief, compliance, propagation, content, delivery) across all CURRENT docs. Frontend at /document-health with IDLE/RUNNING/RESULTS states, 4-stat summary bar, filter bar (status + tier), scorecard table with expandable dimension details, "Create Review Task" for compliance fails, "Edit in Registry" links. Library-wide Findings panel: unmapped beliefs, stage coverage gaps, propagation orphans, never-sent doc list. Sidebar nav entry added.
-   **Phase 013a (PDF Export + Format Version Tracking):** Server-side PDF generation using Puppeteer-core with system Chromium. New `pdfService.ts` with browser singleton, Inter font embedding (base64 woff2), Design Bible HTML template. Schema: 3 new nullable columns on documentsTable (pdf_exported_at, content_updated_at, pdf_file_path). Routes: POST /documents/:id/generate-pdf (renders Puppeteer A4 PDF, saves to disk, sets pdf_exported_at/pdf_file_path, logs to changelog), GET /documents/:id/download-pdf (serves stored PDF). content_updated_at set on PATCH /documents/:id content writes and POST /import/quick-update. Frontend: PDF Export card on document detail sidebar (PDF Available badge, Download/Regenerate buttons, stale-content warning), PDF status column in registry table (printer icon + Current/Stale badge). Dependencies: puppeteer-core, @fontsource/inter, system chromium via Nix.
-   **Phase 013b (Export Fixes):** PDF logo replaced with inline SVG (green accent bar + bold UNLOCK lettermark) in both `pdfService.ts` and `templates/index.ts`. Google Docs export completely overhauled: new AI-powered formatting pipeline (`formatService.ts`) runs document content through Claude with full Design Bible v0.2 rules (typography in pt, colour system, writing rules, terminology, table formatting) before uploading as HTML. Export route now updates existing linked Google Docs via PATCH (was returning "existing" without re-uploading). Static regex-based template (`getGdocsTemplate`) retained as fallback. Frontend: "Push formatted content" button added to Google Docs sidebar card. Design Bible PDF stored in `artifacts/api-server/src/assets/`.
-   **Phase 013c (Google Docs Template + Lead Management):** Google Docs export template rewritten for reliable rendering: native `<h1>`-`<h4>` heading tags (let Google Docs apply its own styles), `bgcolor` attributes for table cells, `<font>` tags for logo text, `<style>` block in `<head>`. Content cleaning: strips internal editing notes (compliance corrections table, designer update brief, ⚠️ markers, strikethrough text, internal notes) before export. AI formatting made optional via `?format=ai` query param (default is fast deterministic template). Export now deletes old Google Doc and creates fresh one (forces proper HTML-to-Docs conversion). Logo uploaded to Drive as PNG and embedded via `<img>` tag with text fallback. Lead Management: DELETE /leads/:id endpoint with transactional changelog, POST /leads/bulk endpoint with pipeline_stage validation and up to 500 leads per upload. Frontend: delete button (trash icon) with confirmation dialog on each lead row, Bulk Upload button with CSV paste dialog (proper quote-aware CSV parsing), live row count preview, and success/error feedback.