...
import { analyzeBeliefs } from "../../lib/beliefAnalysisService";
...
router.post("/leads/:leadId/beliefs/analyze", async (req, res): Promise<void> => {
  ...
  try {
    const result = await analyzeBeliefs(leadId, transcript);
    res.json(result);
  } catch (err: any) {
    res.status(500).json({ error: err.message || "Failed to analyze beliefs" });
  }
});
